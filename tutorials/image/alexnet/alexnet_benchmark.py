...
def inference(images):
  ...
  """
    ...
    pool1 = tf.nn.max_pool(
        lrn1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool1')
    ...
    with tf.name_scope('conv2') as scope:
        ...
        biases = tf.Variable(
            tf.constant(0.0, shape=[192], dtype=tf.float32), trainable=True, name='biases')
        bias = tf.nn.bias_add(conv, biases)
    ...
    pool2 = tf.nn.max_pool(
        lrn2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool2')
    ...
    with tf.name_scope('conv3') as scope:
        ...
        biases = tf.Variable(
            tf.constant(0.0, shape=[384], dtype=tf.float32), trainable=True, name='biases')
        bias = tf.nn.bias_add(conv, biases)
    ...
    pool5 = tf.nn.max_pool(
        conv5, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool5')
...
def run_benchmark():
    ...
    with tf.Graph().as_default():
        ...
        images = tf.Variable(
            tf.random_normal(
                [FLAGS.batch_size, image_size, image_size, 3], dtype=tf.float32, stddev=1e-1))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--num_batches', type=int, default=100, help='Number of batches to run.')
